...
class LerntypRepository
{
    ...
    public function getAllQuestionsFromLerntyp()
    {
        //$query = "SELECT fragetext FROM frage WHERE quiz_id = 16";
        $query = "call corona_prototype.getFrageAndAntwortFromQuizId(16)";
    ...
    {
        $query = "SELECT frage.id_frage, antwortmöglichkeiten FROM antwort LEFT JOIN antwortfrage ON antwort.id_antwort = antwortfrage.antwort_id LEFT JOIN frage ON frage.id_frage = antwortfrage.frage_id WHERE frage.quiz_id = 16";
        $stmt = DB::getInstance()->prepare($query);
        $stmt->execute();
        $result = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    ...
    {
        $sql = "SELECT antwort.id_antwort FROM frage LEFT JOIN antwortfrage ON id_frage = antwortfrage.frage_id LEFT JOIN antwort ON antwortfrage.antwort_id = antwort.id_antwort WHERE antwort.richtigeantwort = 1 AND frage.quiz_id = 16;";
    ...
    {
        $sql = "SELECT id_punktzahlantwort, antworttext FROM punktzahlantwort WHERE ? between punktezahlvon AND puntzahlbis AND quiz_id = 16;";
        $stmt = DB::getInstance()->prepare($sql);
